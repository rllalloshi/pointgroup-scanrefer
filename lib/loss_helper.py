...
def get_loss(data_dict, config, reference=False, use_lang_classifier=False, use_max_iou=False, post_processing=None):
    ...
    # Obj loss
    _, objectness_label, objectness_mask, object_assignment = compute_objectness_loss(data_dict)
    ...
    if reference:
        ...
        objectness_preds_batch = objectness_label.long()#torch.argmax(data_dict['objectness_scores'], 2).long()
