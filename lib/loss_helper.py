...
def get_loss(data_dict, config, reference=False, use_lang_classifier=False, use_max_iou=False, post_processing=None):
    ...
    labels = data_dict["sem_cls_label"].view(
        data_dict["sem_cls_label"].shape[0] * data_dict["sem_cls_label"].shape[1]).long()
    logits = data_dict["object_classifier"].view(
        data_dict["object_classifier"].shape[0] * data_dict["object_classifier"].shape[1],
