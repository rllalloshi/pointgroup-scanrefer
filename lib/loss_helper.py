...
def compute_objectness_loss(data_dict):
    ...
    batch_xyz = torch.from_numpy(data_dict['batch_xyz']).float().cuda()
    gt_center = data_dict['center_label'][:, :, 0:3]
    B = gt_center.shape[0]
    K = batch_xyz.shape[1]
    K2 = gt_center.shape[1]
    dist1, ind1, dist2, _ = nn_distance(batch_xyz, gt_center)  # dist1: BxK, dist2: BxK2
...
def get_loss(data_dict, config, reference=False, use_lang_classifier=False, use_max_iou=False, post_processing=None):
    ...
    # Final loss function
    if use_max_iou:
        loss = 0.5 * objectness_loss + box_loss + 0.1 * sem_cls_loss + 0.1 * ref_loss + lang_loss
    else:
        loss = 0.5*objectness_loss + box_loss + 0.1*sem_cls_loss + 0.01*ref_loss + lang_loss
